...
##            edgeImage = cv2.Canny(image=image, threshold1=CANNY_THRESH_1, threshold2=CANNY_THRESH_2)
            ...
            sigma = 0.33 # percentage of threshold - lower: tighter threshold
                         #                         - higher: wider threshold 

            # get median value of the pixel intensity in the image
            v = np.median(image)

            # make a lower and upper threshold
            lower = int(max(0, (1.0 - sigma) * v))
            upper = int(min(255, (1.0 + sigma) * v))

            # apply the threshold to Canny edge detection
            edgeImage = cv2.Canny(image, lower, upper)

            # taking a matrix of size n,n as the kernel
            kernelSizeRowD = 5
            kernelSizeColD = 5
            kernelDilate = np.ones((kernelSizeRowD, kernelSizeColD), np.uint8)

            kernelSizeRowE = 5
            kernelSizeColE = 5
            kernelErode = np.ones((kernelSizeRowE, kernelSizeColE), np.uint8)

            # use adaptive threshold on result
            edgeImage = cv2.adaptiveThreshold(edgeImage, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 295, 1)
##            self.showImage("Adaptive Image", edgeImage)
##
            # use dilation on result
            edgeImage = cv2.dilate(edgeImage, kernelDilate, 1)
##            self.showImage("Edge Dilate Image", edgeImage)
##
            # use erosion on result
            edgeImage = cv2.erode(edgeImage, kernelErode, 1)
##            self.showImage("Edge Erode Image", edgeImage)
